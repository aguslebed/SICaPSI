...
export default function BuzonEnviados() {
  const { userData } = useUser();

  // Mensajes enviados (carpeta 'sent')
  const sent = useMemo(
    () => (userData?.messages?.items || []).filter((m) => m.folder === "sent"),
    [userData]
  );
        ...
        <div className="overflow-auto border rounded">
          <table className="w-full">
            ...
            <tbody>
              {messages.length === 0 ? (
                <tr>
                  <td colSpan={3} className="p-2 text-center text-gray-500">No hay mensajes enviados</td>
                </tr>
              ) : (
                messages.map((msg) => (
                  <tr
